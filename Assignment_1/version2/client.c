	...
	while(1){
		...
		if (read_bytes == 0){
			...
			if (tmp_user_quit == 0){
				...
				snprintf(print_msg, sizeof(print_msg), " Server shutdown. Press Enter to continue...\n");
				printMsg(stdout, print_msg);
				//shutdown(fd, SHUT_RDWR); //  shutting down and unblocking the other blocked thread
				break;
			}
			else{
				snprintf(print_msg, sizeof(print_msg), " Disconnecting...\n");
				printMsg(stdout, print_msg);
				shutdown(fd, SHUT_RDWR); //  shutting down and unblocking the other blocked thread
				break;
			}
			...
			pthread_mutex_unlock(&disconnect_mutex);
			shutdown(fd, SHUT_RDWR); //  shutting down and unblocking the other blocked thread
			snprintf(print_msg, sizeof(print_msg), " Error with server communication\n");
			printMsg(stdout, print_msg);
			//shutdown(fd, SHUT_RDWR); //  shutting down and unblocking the other blocked thread
			break;
		}
...
int main()
{
	...
	if (pthread_create(&read_thread, NULL, receive_msg_thread_func, &socket_fd) != 0){
	...
	pthread_join(read_thread, NULL); // wait for the receive message thread to finish
	close(socket_fd);
